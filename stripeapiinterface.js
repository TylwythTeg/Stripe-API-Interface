...
import {API_RESOURCES} from './api_resources.js';
...
function runApiInterface() {
    ...
    apiObjects.addEventListener("change", function(event) {
        ...
        var resource = Object.keys(API_RESOURCES[selection]);

        // unset auth_as by default
        checkbox.checked = false;
        checkbox.dispatchEvent(new Event('change'));

        populateMethodSelection(API_RESOURCES[selection], resource);
    ...
    function updateRequestArea(textArea, apiObject, apiMethod) {
        const resource = API_RESOURCES[apiObject][apiMethod];

        /* Show / Hide JSON Request Area */
        const jsonToggle = resource.params ? "remove" : "add";
        textArea.classList[jsonToggle]("removed");
        ...
        const fileUploadToggle = resource.file_upload ? "add" : "remove";
        uploadContainer.classList[fileUploadToggle]("show");
    ...
    function updateLink(linkElement, apiObj, apiMethod) {
        var resource = API_RESOURCES[apiObj][apiMethod];
    ...
    function setRelevantTemplateOptions() {
        ...
        var templates = TEMPLATES[apiObjects.value];
        if (!templates) {return;}

        var relevant = false;
        Object.keys(templates).forEach(function(template) {
            if (templates[template].method === apiMethods.value) {
                relevant = true;
            }
        });
        var relevantOptions = [];
        ...
        [...templateSelections2.children].forEach((option, index) => {
            var template = TEMPLATES.searchByName(option.value);
    ...
    apiMethods.addEventListener("change", function(event) {
        ...
        var names = Object.keys(templates).filter(name => {
    ...
    function updateEndpointContainer(container, resource, method) {
        ...
        wrapper.innerHTML = '';
        var url = API_RESOURCES[resource][method].endpoint;
        //alert(url);
    ...
    function isResource(str) {
        return Object.keys(API_RESOURCES).includes(str);
    }
    ...
    submitButton.addEventListener("click", function(event) {
        ...
        var met = API_RESOURCES[apiObjects.value][apiMethods.value];
        var type = API_RESOURCES[apiObjects.value][apiMethods.value].type;
    ...
    function buildEndpoint() {
        ...
        const resource = API_RESOURCES[apiObjects.value][apiMethods.value];
        let newEndpoint = resource.endpoint;

        const inputs = document.getElementsByClassName("object_id_input");

        for (var i = 0; i < inputs.length; i++) {
            const input = inputs[i];
            const objPrompt = input.getAttribute("placeholder");
    ...
    */
    ...
    var resource = Object.keys(API_RESOURCES[apiObjects.value]);
    populateMethodSelection(API_RESOURCES[apiObjects.value], resource);
    updateEndpointContainer(endpointContainer, apiObjects.value, apiMethods.value);
    updateRequestArea(requestContainer, apiObjects.value, apiMethods.value);
    updateLink(docLink, apiObjects.value, apiMethods.value);
    initTemplateList();
